...
function makeTextEvent(content: string): ChatEvent {
  return { id: `evt-${Math.random().toString(36).slice(2)}`, type: "text", content };
}

function makeResultEvent(
  overrides: Partial<import("~/stores/chat-types").ResultEvent> = {},
): ChatEvent {
  return {
...
function makeToolUseEvent(
  overrides: Partial<import("~/stores/chat-types").ToolUseEvent> = {},
): ChatEvent {
  return {
    id: `evt-${Math.random().toString(36).slice(2)}`,
    type: "tool_use",
...
function makeRateLimitEvent(
  overrides: Partial<import("~/stores/chat-types").RateLimitEvent> = {},
): ChatEvent {
  return {
    id: `evt-${Math.random().toString(36).slice(2)}`,
    type: "rate_limit",
...
function makeStreamEvent(): ChatEvent {
  return { id: `evt-${Math.random().toString(36).slice(2)}`, type: "stream" };
}

function makeCompactStatusEvent(status: string): ChatEvent {
  return { id: `evt-${Math.random().toString(36).slice(2)}`, type: "compact_status", status };
}

function makeCompactBoundaryEvent(
  overrides: Partial<import("~/stores/chat-types").CompactBoundaryEvent> = {},
): ChatEvent {
  return {
    id: `evt-${Math.random().toString(36).slice(2)}`,
...
describe("chat-store", () => {
  ...
  describe("handleServerEvent", () => {
    ...
    it("appends text event to streamingEvents (not turn.events) during streaming", () => {
      useChatStore.getState().handleServerEvent("sess-1", makeTextEvent("Hello"));
      const s = useChatStore.getState().sessions["sess-1"];
      // During streaming, events go to the buffer; turn.events stays empty.
      expect(s?.turns[0]?.events).toHaveLength(0);
      expect(s?.streamingEvents).toHaveLength(1);
      const first = s?.streamingEvents[0];
      if (first?.type === "text") {
        expect(first.content).toBe("Hello");
      }
    });

    it("merges streamingEvents into turn.events on result", () => {
      useChatStore.getState().handleServerEvent("sess-1", makeTextEvent("Hello"));
      useChatStore
        .getState()
        .handleServerEvent("sess-1", makeResultEvent({ stopReason: "end_turn" }));
      const s = useChatStore.getState().sessions["sess-1"];
      // After result: buffer merged into turn, buffer cleared.
      expect(s?.turns[0]?.events).toHaveLength(2);
      const firstEvent = s?.turns[0]?.events[0];
      if (firstEvent?.type === "text") {
        expect(firstEvent.content).toBe("Hello");
      }
      expect(s?.turns[0]?.events[1]?.type).toBe("result");
      expect(s?.turns[0]?.complete).toBe(true);
      expect(s?.streamingEvents).toHaveLength(0);
    });
    ...
    it("ignores rate_limit events (no turn append)", () => {
      useChatStore
        .getState()
        .handleServerEvent("sess-1", makeRateLimitEvent({ status: "warning", utilization: 0.8 }));
      const turns = useChatStore.getState().sessions["sess-1"]?.turns;
    ...
    it("ignores stream events", () => {
      useChatStore.getState().handleServerEvent("sess-1", makeStreamEvent());
      const turns = useChatStore.getState().sessions["sess-1"]?.turns;
      expect(turns?.[0]?.events).toHaveLength(0);
    });

    it("sets compacting flag on compact_status", () => {
      useChatStore.getState().handleServerEvent("sess-1", makeCompactStatusEvent("compacting"));
    ...
    it("preserves context usage on compact_boundary (avoids flash)", () => {
      ...
      useChatStore
        ...
        .handleServerEvent(
          "sess-1",
          makeCompactBoundaryEvent({ trigger: "auto", preTokens: 50000 }),
        );
  ...
  describe("setSessions stale overwrite protection", () => {
    it("preserves live state when existing gitVersion >= incoming", () => {
      useChatStore
        .getState()
        .addSession(makeMeta({ gitVersion: 3, state: "running", connected: true }));

      // Simulate stale session.list response with lower gitVersion
      useChatStore
        .getState()
        .setSessions([makeMeta({ gitVersion: 2, state: "idle", connected: false })], "proj-1");
    ...
    it("accepts incoming state when gitVersion is higher", () => {
      ...
      useChatStore.getState().setSessions([makeMeta({ gitVersion: 5, state: "idle" })], "proj-1");
    ...
    it("accepts incoming state when existing gitVersion is 0 (initial load)", () => {
      ...
      useChatStore
        .getState()
        .setSessions([makeMeta({ gitVersion: 2, state: "running" })], "proj-1");
      ...
      useChatStore.getState().setSessionHistory("sess-1", [
        {
          ...
          events: [
            ...
            makeResultEvent({ stopReason: "end_turn" }),
          ],
          complete: true,
        },
      ]);
