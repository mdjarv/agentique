...
export function PromptGroupProvider({
  ...
  const startAll = useCallback(() => {
    ...
    if (swarmEligible.length > 0) {
      ...
      const channelName = parent?.name ? `${parent.name} channel` : "Swarm";

      void (async () => {
        try {
          const result = await createSwarm(ws, projectId, channelName, members, sessionId);
          ...
          if (result.errors?.length) {
            ...
            toast.success(`Team "${channelName}" created with ${swarmEligible.length} sessions`);
          }
        } catch (err) {
