...
interface SwarmComposerProps {
  ...
  onCreated: (channelId: string, firstSessionId: string) => void;
}
...
export function SwarmComposer({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result = await createSwarm(ws, projectId, channelName, members);

      if (result.errors?.length) {
        toast.warning(`Channel created with ${result.errors.length} warning(s)`);
      } else {
        toast.success(`Channel "${channelName}" created with ${members.length} sessions`);
      }
      ...
      {mode === "goal" && (
        <div className="space-y-2">
          <Textarea
            ...
            placeholder="Describe the goal — all agents receive this prompt and coordinate via their channel."
