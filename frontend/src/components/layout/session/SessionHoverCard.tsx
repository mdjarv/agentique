        ...
        case "needs_rebase":
          toast.warning("Branches have diverged — rebase needed before merge", {
            ...
            action: {
              label: "Rebase & Merge",
              onClick: async () => {
                const rb = await rebaseSession(ws, sessionId);
                if (rb.status !== "rebased") {
                  toast.error(
                    rb.status === "conflict"
                      ? "Rebase conflicts"
                      : "error" in rb
                ...
                const mr = await mergeSession(ws, sessionId, mode);
                if (mr.status === "merged") toast.success("Rebased and merged");
                else toast.error("error" in mr ? mr.error : `Merge failed: ${mr.status}`);
              },
            },
          });
          break;
