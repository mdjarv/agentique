...
export function useUncommittedDiff(sessionId: string) {
  ...
  const fetchUncommittedDiff = useCallback(async () => {
    ...
    try {
      ...
      if (prevSessionId.current === sessionId) {
        toast.error(getErrorMessage(err, "Failed to load uncommitted diff"));
      }
      return null;
    } finally {
      if (prevSessionId.current === sessionId) {
  ...
  // Initial fetch when mounting a non-running session that has dirty files.
  const didInitialFetch = useRef(false);
  useEffect(() => {
    if (prevSessionId.current !== sessionId) didInitialFetch.current = false;
  }, [sessionId]);
  useEffect(() => {
    if (!didInitialFetch.current && !isRunning && isDirty) {
