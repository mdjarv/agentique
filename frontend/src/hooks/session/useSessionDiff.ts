...
export function useSessionDiff(sessionId: string) {
  ...
  // Initial fetch when mounting a non-running session that has commits ahead.
  const didInitialFetch = useRef(false);
  useEffect(() => {
    if (prevSessionId.current !== sessionId) didInitialFetch.current = false;
  }, [sessionId]);
  useEffect(() => {
    if (!didInitialFetch.current && !isRunning && !isMerged) {
      didInitialFetch.current = true;
      fetchDiff();
    }
  }, [isRunning, isMerged, fetchDiff]);

  // Fetch diff when session transitions from running to idle
  useEffect(() => {
