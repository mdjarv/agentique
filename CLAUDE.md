...
- **claude** (default) — `runtime/cli/claude` adapter over `claudecli-go`. Full feature set: resume, fork, mid-turn `SendMessage`, plan mode, thinking, subagent events, rate-limit / compaction events, live MCP reconnect, tool-progress ticks.
...
**Known gaps vs. pre-migration claude behavior** (see `docs/tech-debt.md`): `AgentResult` metadata doesn't surface today because the neutral event set omits it. The new delta event types (`ToolOutputDeltaEvent`, `ReasoningDeltaEvent`, `TurnDiffEvent`, `ToolProgressEvent`) are wired through the backend pipeline but have no frontend renderers yet — they're classified as transient/skip.
