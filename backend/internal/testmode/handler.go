...
func (h *Handler) HandleSeed(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		httperror.RespondError(w, httperror.BadRequest(fmt.Sprintf("invalid JSON: %v", err)))
		return
	}
	...
	for _, p := range req.Projects {
		...
		if _, err := h.Queries.CreateProject(ctx, store.CreateProjectParams{
			...
			httperror.RespondError(w, httperror.Internal(fmt.Sprintf("create project %s", p.ID), err))
			return
		}
	}
	...
	for _, s := range req.Sessions {
		...
		if s.Live {
			...
			if err != nil {
				httperror.RespondError(w, httperror.Internal(fmt.Sprintf("create live session %s", s.ID), err))
				return
			}
			...
			if _, err := h.Queries.CreateSession(ctx, store.CreateSessionParams{
				...
				httperror.RespondError(w, httperror.Internal(fmt.Sprintf("create session %s", s.ID), err))
				return
			}
		}
	}

	httperror.JSON(w, http.StatusOK, SeedResult{
...
func (h *Handler) HandleInjectEvent(w http.ResponseWriter, r *http.Request) {
	...
	if mockSess == nil {
		httperror.RespondError(w, httperror.NotFound(fmt.Sprintf("no mock session for %s", req.SessionID)))
		return
	}
	...
	if err := mockSess.InjectEvent(event); err != nil {
		httperror.RespondError(w, httperror.Internal("inject event", err))
		return
	}

	httperror.JSON(w, http.StatusOK, map[string]string{"status": "ok"})
}
...
func (h *Handler) HandleReset(w http.ResponseWriter, r *http.Request) {
	...
	ctx := context.Background()
	tables := []string{"session_events", "sessions", "channels", "projects"}
...
func (h *Handler) HandleState(w http.ResponseWriter, r *http.Request) {
	...
	httperror.JSON(w, http.StatusOK, states)
}
...
func (h *Handler) HandleExportSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httperror.RespondError(w, httperror.NotFound(fmt.Sprintf("session %s not found", sessionID)))
		return
	}
	...
	if err != nil {
		httperror.RespondError(w, httperror.Internal("list events", err))
		return
	}
	...
	httperror.JSON(w, http.StatusOK, export)
}
