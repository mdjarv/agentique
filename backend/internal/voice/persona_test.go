...
func TestZeroPersonaKeepsTheSafetyRules(t *testing.T) {
	got := strings.ToLower(SystemInstruction(Briefing{Persona: Persona{}.Sanitize()}))
...
func TestPersonalityCannotOverrideTheRules(t *testing.T) {
	...
	got := SystemInstruction(Briefing{Persona: hostile})
...
func TestVerbosityChangesTheInstruction(t *testing.T) {
	brief := SystemInstruction(Briefing{Persona: Persona{Verbosity: VerbosityBrief}.Sanitize()})
	detailed := SystemInstruction(Briefing{Persona: Persona{Verbosity: VerbosityDetailed}.Sanitize()})
	if brief == detailed {
